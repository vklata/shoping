...
router.post('/loginuser', [
    ...
    try {
        ...
        if (!user) {
            return res.status(400).json({error: "Try Logging in with correct email" });
        }

        const pwdCompare = await bcrypt.compare(req.body.password, user.password); // this return true false.
        if (!pwdCompare) {
